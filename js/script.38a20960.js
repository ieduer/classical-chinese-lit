...
function syncAiConversation(reason = 'update') {
    ...
    const scope = window.BdfzLearningRecords?.scope;
    if (!scope || aiConversation.some(message => message.captureScope !== scope)) return;
...
function pushAiConversation(role, content, reason = 'update', operation = {}) {
    const normalizedRole = role === 'assistant' || role === 'system' ? role : 'user';
    const text = String(content || '').trim();
    if (!text) return;
    aiConversation.push({ role: normalizedRole, content: text, id: operation.operationId, createdAt: operation.occurredAt, captureScope: window.BdfzLearningRecords?.scope || null });
...
async function handleAISend() {
    ...
    let userQuery = aiInputElement.value;
    const selectionToSend = currentSelection;
    if (!userQuery.trim() && !selectionToSend) return;
    if (!currentPoemObject) { appendAIMessage('请先选择一篇文章。', 'system error'); return; }
    if (!userQuery.trim()) userQuery = `解释一下这段文字：“${selectionToSend}”`;
    const poem = currentPoemObject;
    const payload = { selectedText: selectionToSend, poemContext: poem, explicitQuery: userQuery };
    let capture;
    try { capture = window.MfLearningChat.capture(payload, aiConversationSessionKey); }
    catch (_) { appendAIMessage('学习记录尚未准备好，请稍后重试。', 'system error'); return; }
    aiSendBtn.disabled = true;
    appendAIMessage(userQuery, 'user');
    if (selectionToSend) appendAIMessage(`(针对选中文字: “${selectionToSend}”)`, 'system info');
    const thinking = appendAIMessage('窺視者思考中...', 'ai-thinking');
    try {
        const result = await window.MfLearningChat.send(capture, callAIWorker);
        if (!result.visible || currentPoemObject !== poem) return;
        aiInputElement.value = ''; currentSelection = '';
        aiInputElement.style.height = 'auto'; aiInputElement.dispatchEvent(new Event('input'));
        if (result.text) {
            appendAIMessage(result.text, 'ai');
            // Preserve the legacy summary while the detailed immutable journal owns full history.
            pushAiConversation('user', userQuery, 'user-message', capture.answer);
            pushAiConversation('assistant', result.text, 'assistant-message', result.reply);
        } else appendAIMessage(result.error, 'system error');
    } catch (_) {
        if (capture.current() && currentPoemObject === poem) appendAIMessage('记录尚未保存完整，请使用「重试保存」。不会自动重新调用模型。', 'system error');
    } finally {
        thinking?.remove(); aiSendBtn.disabled = false;
        if (capture.current()) aiInputElement.focus();
    }
}
...
async function callAIWorker(payload) {
    ...
    try {
        const response = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), signal: controller.signal });
        if (!response.ok) throw Object.assign(new Error(`HTTP ${response.status}`), { status: response.status, responseBody: await response.text() });
        return await response.json();
    } finally { window.clearTimeout(timeoutId); }
}

function setupLearningRecords() {
    const s = window.BdfzLearningRecords, status = document.getElementById('learning-record-status');
    if (!s) return;
    s.onState(state => { if (status) status.textContent = state.status === 'synced' ? '学习记录已同步' : state.status === 'saved_locally' ? '学习记录已保存在此设备' : state.status === 'unattributed' ? '未确认登录，记录仅留在此设备' : '学习记录待同步'; });
    void s.prepare().catch(() => {});
    document.getElementById('learning-record-retry')?.addEventListener('click', () => { void window.MfLearningChat.retryStorage().catch(() => { if (status) status.textContent = '保存尚未完成，请保留此页面后重试'; }); });
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.addEventListener('bdfz:session-invalidated', () => { aiConversation = []; resetAiConversationSession(); aiMessagesElement?.replaceChildren(); if (aiInputElement) aiInputElement.value = ''; currentSelection = ''; });
